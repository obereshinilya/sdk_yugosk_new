        ...
        <div class="doc_header" style="padding-bottom: 6px">
            <table>
                ...
                <tr>
                    ...
                    <td>
                        @can('entries-add')
                            <div class="bat_add"><a href="/docs/directory_tb/create">Добавить ТБ</a></div>
                        @endcan
                    </td>
                </tr>
                </tbody>
            ...
            <div style="background: #FFFFFF; border-radius: 6px" class="row_block form51">
                <table id="table_for_search" style="display: table; table-layout: fixed">
                    ...
                    <tr>
                        ...
                        <th style="width: 15%">Филиал ДО</th>
                        <th style="width: 5%">ОПО</th>
                        <th style="width: 10%">Элемент ОПО</th>
                        <th style="width: 35%" colspan="2">ТБ элемента ОПО</th>
                    ...
                    @foreach ($tb as $row)
                        <tr>
                            <td style="text-align: center">{{ $row->id_tb }}</td>
                            <td style="text-align: center">{{ $row->short_name_do }}</td>
                            <td style="text-align: center" class="name_event">{{ $row->short_name_opo }}</td>
                            <td style="text-align: center" class="name_event">{{ $row->short_name_obj }}</td>
                            <td style="text-align: center" class="name_event">{{ $row->full_name_type }}</td>
                            <td style="text-align: center" class="name_event">{{ $row->full_name_tb }}</td>
                            @if($row->type_tb == 2 || $row->type_tb == 12)
                            ...
                            @can('entries-edit')
                                <td class="centered" style="text-align: center">
                                    <a href="/docs/directory_tb/edit/{{$row->id_tb}}"><img alt=""
                                                                                           src="{{asset('assets/images/icons/edit.svg')}}"
                                                                                           class="check_i"
                                                                                           style="margin-left: 30px"></a>
                                    {{--                                <a href="/docs/directory_tb/open/{{$row->id_tb}}"><img alt="" src="{{asset('assets/images/icons/search.svg')}}" class="open_i" style="margin-left: 10px"></a>--}}
                                </td>
                            @endcan
                        </tr>
                    @endforeach
